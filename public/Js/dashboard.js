...
(() => {
  ...
  // Create / reuse badge strip above table
  ...
  const addOption = (s,v,l,extra={}) => {
    const opt = document.createElement("option");
    opt.value=v; opt.textContent=l;
    Object.entries(extra).forEach(([k,val])=>opt.dataset[k]=val);
    s.appendChild(opt);
  };
  ...
  const field = (o,p,d=null)=> {
    try { return p.split(".").reduce((a,k)=>(a&&k in a?a[k]:undefined),o) ?? d; }
    catch{ return d; }
  };
  ...
  async function fetchJSON(url, opts={}) {
    ...
    if(!res.ok){
      ...
      throw new Error(`${res.status} ${res.statusText}: ${text||"failed"}`);
    }
    return res.json();
  }
    ...
    if(list.length===0){
      try{
        ...
        for(const it of items){
          ...
          list.push({
            id, name:String(it.name||"").trim(), segment:seg,
  ...
  const computeATM=(strikes,spot)=>{
    ...
    let best=strikes[0],diff=Math.abs(strikes[0]-spot);
    for(let i=1;i<strikes.length;i++){
      const d=Math.abs(strikes[i]-spot);
      if(d<diff){best=strikes[i];diff=d;}
    }
    return best;
  };
  ...
  async function fetchAndRenderChain(){
    ...
    try{
      const j=await fetchJSON(url);
      if(j.status && j.status!=="success") throw new Error(j.detail||"backend != success");
